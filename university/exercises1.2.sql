...
select *
from students s left join student_grades sm on s.student_id=sm.student_id
...
select m.module_id, module_name, student_id
from modules m
...
select m.module_id, module_name, e.surname, lecturer_id, student_id
from modules m
  left join students_modules sm on sm.module_id = m.module_id
  left join employees e on m.lecturer_id=e.employee_id
-- nie inner join,poniewaz w modules lecturer_id moze byc null i mogloby ich nie dolaczyc innerem
where student_id is null
...
select employee_id, surname, first_name, lecturer_id
from employees
...
select s.student_id, first_name, surname, group_no, module_id
from students s
...
select distinct s.student_id, first_name, surname, exam_date
from students s
...
select module_name, no_of_hours, lecturer_id, first_name
from modules m
  left join employees e on m.lecturer_id=e.employee_id
...
select s.first_name, s.surname, module_name, e.surname lecturer_surname, l.department
from students s
...
select sum(no_of_hours) sum_hours
from modules m
  left join lecturers l on l.lecturer_id=m.lecturer_id
